...
for( my $i = 0; $i <= $#toRead; $i++){
	...
	my $commando = "prun --bexec \"make\" --exec \"corra \%IN\" --outputs file.root --athenaTag=16.0.3.5 --nFilesPerJob=1 --extFile=root,h --excludeFile=.svn --inDS $line --outDS $prefix.SUSYFILE.muon.$yymmdd.run$runnumber.V$version --excludedSite=ANALY_SWT2_CPB\n";
	#print "$index";
	print $commando;
	system $commando;
	#print $line;
