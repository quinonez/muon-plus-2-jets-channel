...
for( my $i = 0; $i <= $#toRead; $i++){
	...
	my $commando = "prun --bexec \"make\" --exec \"corra \%IN\" --outputs file.root --athenaTag=16.0.3.5 --nFilesPerJob=1 --extFile=root,h --excludeFile=.svn --inDS $line --outDS $prefix.SUSYFILE.muon.$yymmdd.MC$mccode.V$version --excludedSite=ANALY_BNL_ATLAS_1,ANALY_CERN,ANALY_DESY-HH,ANALY_TW-FTT\n";
	#print "$index";
	print $commando;
	system $commando;
	#print $line;
